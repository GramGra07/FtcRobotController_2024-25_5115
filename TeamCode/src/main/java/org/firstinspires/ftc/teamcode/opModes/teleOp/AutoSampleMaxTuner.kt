...
import org.firstinspires.ftc.teamcode.utilClass.varConfigurations.AutoVars.scoringOffsetTurn
...
//            //////////
            ...
            var turnAngle = pose2.angleTo(pose) + Math.toRadians(AutoVars.sampleCenterTurnOffset)

            var offset = AutoVars.sampleCenterPositionOffset
            ...
            finalOffsets.put("extensionGainC", extensionGain+AutoVars.sampleCenterPositionOffset)
            finalOffsets.put("turnGainC", turnGain+AutoVars.sampleCenterTurnOffset)
            resetGain()
            ...
            ////////
            runAction = true
             pose = Pose2d(-48.0, -25.0, Math.toRadians(90.0))
             pose2 = Pose2d(Vector2d(pose.position.x,-45.0), lastPose.heading.toDouble())
             turnAngle = pose2.angleTo(pose) + Math.toRadians(AutoVars.sampleRightTurnOffset)

             offset = AutoVars.sampleRightPositionOffset
...
//                            robot.tempAuto.resetVar(TempAutoDAFix.DATypes.PICKUP_E)
            ...
            finalOffsets.put("extensionGainR", extensionGain+AutoVars.sampleRightPositionOffset)
            finalOffsets.put("turnGainR", turnGain+AutoVars.sampleRightTurnOffset)
            resetGain()
...
//            //////////
            ...
            turnAngle = pose2.angleTo(pose) + Math.toRadians(AutoVars.sampleLeftTurnOffset)
            pose2 = Pose2d(Vector2d(-58.0,-45.0), lastPose.heading.toDouble())
            ...
            finalOffsets.put("extensionGainL", extensionGain+AutoVars.sampleLeftPositionOffset)
            finalOffsets.put("turnGainL", turnGain+AutoVars.sampleLeftTurnOffset)
            resetGain()
//
            runBlocking(
                SequentialAction(
