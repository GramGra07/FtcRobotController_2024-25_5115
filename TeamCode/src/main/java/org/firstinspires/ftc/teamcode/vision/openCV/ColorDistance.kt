...
class ColorDistance(var color: String) : OpenCvPipeline() {
    ...
    override fun processFrame(input: Mat): Mat {
        ...
        if (boundRect.size > 0) {
            ...
            Imgproc.rectangle(
                input,
                boundRect[highIndex]!!.tl(),
                boundRect[highIndex]!!.br(),
                scalarVals(color),
                2
            )
        }
    ...
    data class distanceRecognizerDetails(
        val height: Double,
        val width: Double,
        val inToPixels: Double,//pix/in
    ) {
        val pixToIn = 1 / inToPixels
        fun convertPixToIn(pix: Double): Double {
            return pix * pixToIn
        }

        fun convertInToPix(inches: Double): Double {
