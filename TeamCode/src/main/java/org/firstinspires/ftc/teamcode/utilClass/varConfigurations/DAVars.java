package org.firstinspires.ftc.teamcode.utilClass.varConfigurations;

import com.acmerobotics.dashboard.config.Config;

@Config
public class DAVars {
    public static double collapseE = 0;
    public static double collapseP = 0;
    public static double hSpecimenE = 1450;
    public static double hSpecimenP = 700;
    public static double hBasketE = 2900;
    public static double hBasketP = 1725;//1850
    public static double pickUpE = 1800;
    public static double pickUpP = 0;
    public static double humanE = 500;
    public static double humanP = 250;
    public static double liftNeg = 1200;
    public static double liftP = 1600;
}
