...
class KeybindMaker:
    def __init__(self):
        ...
        self.keyOptions = ["dpad_down", "dpad_up", "dpad_left", "dpad_right", "left_bumper",
                           "right_bumper", "touchpad",
                           "left_stick_button",
        ...
        self.boolKeyOptions = ["dpad_down", "dpad_up", "dpad_left", "dpad_right", "left_bumper",
                               "right_bumper",
                               "touchpad",
                               "left_stick_button",
                               "right_stick_button", "circle", "triangle", "square", "cross"]
        self.nonBoolKeyOptions = ["left_trigger", " right_trigger"]
        self.directDriveKeyOptions = ["left_stick_x", "left_stick_y", "right_stick_x",
                                      "right_stick_y"]

        self.current_function_index = 0
        self.current_function_label = tk.Label(self.root,
                                               text=f"Assigning keybind for {self.functions[0]}")
    ...
    def assign_keybind(self, button):
        ...
        self.list[self.functions[self.current_function_index]] = button.cget('text').replace(
            "Assign ", "")
